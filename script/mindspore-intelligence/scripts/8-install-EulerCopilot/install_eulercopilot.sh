...
YAML_EXTRACTOR="${DEPLOY_DIR}/scripts/4-deploy-deepseek/yaml_extractor.py"
...
modify_yaml() {
    ...
    # 验证参数获取
    if [ -z "$host" ] || [ -z "$port" ] || [ -z "$model_name" ]; then
    ...
    if [[ "$preserve_models" != [Yy]* ]]; then
        set_args+=(
            "--set" "models.answer.endpoint=http://$host:$port/v1"
            "--set" "models.answer.name=${model_name}"
	    ...
	    "--set" "models.embedding.endpoint=http://$host:11435/v1"
...
main() {
    ...
    local arch internal_host

    arch=$(get_architecture) || exit 1
    internal_host=$(get_network_ip) || exit 1
