...
def execute_command(cmd_list):
    ...
    Returns:
        The output results of the executed command.
    """
    try:
...
def get_numa_map(affinity: bool, core_num_per_workqueue: int):
    """
    Derive the npu-numa affinity map

    Args:
        affinity (`bool`):
            Affinity mode that binds the cann queue into its affiliative NUMA node.
    ...
    Returns:
        The map of each npu to the last CPU core of its corresponding NUMA node.
    """
...
npu_device_dict = {"d801":"910","d802":"910B","d803":"910C","d100":"310","d500":"310P"}

def check_device():
    """
    Check the device type of target platform
    """
    device_type = os.getenv("DEVICE_TYPE", "NPU")
    if device_type != "NPU":
        #logger.info(f"Current platform {device_type} not supported")
        print(f"Current platform {device_type} not supported")
        return False
    ...
    if npu_device_type != "910B" and npu_device_type != "910A":
        ...
        print(f"Current platform {npu_device_type} not supported")
        return False
...
binding_cann_workqueue(2, 8, True)
